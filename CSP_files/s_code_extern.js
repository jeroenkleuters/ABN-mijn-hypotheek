var listOfExceptionSites = {
        'www.uitgesprokentalent.nl' : "uitgesprokentalent",
        'acc.uitgesprokentalent.nl' : "uitgesprokentalent-et",
        'berendb.nl' : "berendb",
        'acc.berendb.bitlibre.nl' : "berendb-et",
        'test.berendb.bitlibre.nl' : "berendb-et"
    },
    locationHost = window.location.host, s_account;

if (typeof(s_account) == "undefined") {
	for (var key in listOfExceptionSites) {
		if (locationHost.indexOf(key) !== -1){
			s_account="abnamro" + listOfExceptionSites[key];
			break;
		}else{
			s_account="abnamronl";
		}
...
/**version 3 March 2014 GB 13:30 **/

function setProtocol(){
	var strProtocol = window.location.protocol;
	var i = strProtocol.indexOf(":");
	if (i != -1) {
		strProtocol = strProtocol.substring(0,i);
	}
	s.prop9=strProtocol;
}

function setExternalCampaign(){
	var ext = s.getQueryParam('ext');
...
function setInternalCampaign(){
	//internal campaigns
    var pos = s.getQueryParam('pos');
...
s.linkInternalFilters="javascript:,abnamro.nl,abnamro.tv,gripoprisicos.nl,abnamro-basishypotheek.nl,"+window.location.host;
...
s.trackingServer="abnamro.122.2o7.net"
...
/* time parting v3.2 configuration, Europe */
s._tpDST = {
2012:'3/25,10/28',
2013:'3/31,10/27',
2014:'3/30,10/26',
...
function s_doPlugins(s) {
	...
	s.prop25 = s.eVar40 = tpA[2]; // Day of week
	s.prop26 = s.eVar41 = tpA[1]; // Time at 30 minute level
...
+"da=['Zondag','Maandag','Dinsdag','Woensdag','Donderdag','Vrijdag','Za"
+"terdag'],d=new Date(),a=[];z=z?z:0;z=parseFloat(z);if(s._tpDST){var"
